    ...
    if gcd(key1,26) == 1:
        while i < len(plainText):
            ...
            if char >= 65 and char <= 90:
                char = (char%65 * key1)%26
                char = (char + key2)%26 + 65

            elif char >= 97 and char <= 122:
                char = (char%97 * key1)%26
                char = (char + key2)%26 + 97
...
def decrypt_affine_cipher(encryptedText, key1, key2):
    i = 0
    decryptedText = ""
    inverseOfKey = take_inverse(26,key1)

    while i < len(encryptedText):
        char = ord(encryptedText[i])
        if char >= 65 and char <= 90:
            char = (char%65 - key2)%26
            char = (char * inverseOfKey)%26 + 65

        elif char >= 97 and char <= 122:
            char = (char%97 - key2)%26
            char = (char * inverseOfKey)%26 + 97
